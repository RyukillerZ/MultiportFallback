...
fun_bot1() {
			[[ "$(grep -wc "RyukillerZbot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS RyukillerZbot backup_bot' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS RyukillerZbot backup_bot >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "RyukillerZbot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "RyukillerZbot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "RyukillerZbot" /etc/rc.local) != '0' ]] && {
                sed -i '/RyukillerZbot/d' /etc/rc.local
            }
...
echo -e "\033[0;34m------------------------------------\033[0m"
echo -e ""
echo -e "Default Password Autobackup : AutobackupData"
echo -e ""
read -p " Link File: " url
read -p " Password File: " InputPass
if [[ -z $InputPass ]]; then
InputPass=AutobackupData
fi
